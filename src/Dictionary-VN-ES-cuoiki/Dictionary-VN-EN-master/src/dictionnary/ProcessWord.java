...
public class ProcessWord  {
    public static HashMap <String, Meanings> listEn_Vn = new HashMap<String, Meanings>(); //HashMap được sử dụng để lưu trữ các phần tử 
    //dưới dạng "key/value". Key và value là kiểu dữ liệu bất kỳ, và bạn có thể try cập các giá trị của HasMap bằng một key cụ thể.
    ...
    */
    public void ReadFile_EN_VN(){
        ...
        try{
            ...
            DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance(); //đoc tâpj tin
            DocumentBuilder dBuilder = dbFactory.newDocumentBuilder(); //phân tích thông tin
            Document doc  = dBuilder.parse(En_Vn_XML);
            
            doc.getDocumentElement().normalize(); //In ra man hinh ten cua phan tu goc cua tai lieu XML xmlFile.xml la g
            
            NodeList list = doc.getElementsByTagName("ghi lại");
            for(int i = 0; i < list.getLength(); i++){
                ...
                if(node.getNodeType() == Node.ELEMENT_NODE){
                    ...
                    String keyword;
                   //lấy từ tiếng anh
                    keyword = elm.getElementsByTagName("word").item(0).getTextContent().toString(); //đầu vào từ
                    listKey_EN_VN.add(keyword);
                    String waitpro;
                    Meanings temp;
                    waitpro = elm.getElementsByTagName("meaning").item(0).getTextContent().toString();//ý nghĩa từ
                    /*
